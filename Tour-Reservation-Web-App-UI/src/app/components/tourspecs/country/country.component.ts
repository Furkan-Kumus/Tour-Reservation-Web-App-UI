...
export class CountryComponent implements OnInit {
  ...
  async getCountry() {
    const allCountry: { totalCount: number; countries: List_Country[] } = await this.countryService.read(this.paginator ? this.paginator.pageIndex : 0, this.paginator ? this.paginator.pageSize : 5)
    this.dataSource = new MatTableDataSource<List_Country>(allCountry.countries);
