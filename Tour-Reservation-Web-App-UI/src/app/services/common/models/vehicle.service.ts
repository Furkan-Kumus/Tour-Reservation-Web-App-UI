...
export class VehicleService {
  ...
  async read(page: number = 0, size: number = 5, successCallBack?: () => void, errorCallBack?: (errorMessage: string) => void): Promise<{ totalCount: number; vehicles: List_Vehicle[] }> {
    const promiseData: Promise<{ totalCount: number; vehicles: List_Vehicle[] }> = this.httpClientService.get<{ totalCount: number; vehicles: List_Vehicle[] }>({
